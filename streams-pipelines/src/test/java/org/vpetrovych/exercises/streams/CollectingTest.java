...
public class CollectingTest {
    ...
    Collecting testObject = new Collecting();

    @Test
    public void testSum() {
        assertEquals(15, testObject.sum(IntStream.iterate(1, i -> i + 1).limit(5)));
        assertEquals(10, testObject.sum(IntStream.iterate(0, i -> i + 1).limit(5)));
        assertEquals(1, testObject.sum(IntStream.iterate(1, i -> -i).limit(5)));
        assertEquals(-2, testObject.sum(IntStream.iterate(0, i -> -(i + 1)).limit(5)));
        assertEquals(-14, testObject.sum(IntStream.iterate(0, i -> -2).limit(8)));

    }

    @Test
    public void testProd() {
        assertEquals(120, testObject.production(IntStream.iterate(1, i -> i + 1).limit(5)));
        assertEquals(0, testObject.production(IntStream.iterate(0, i -> i + 1).limit(5)));
        assertEquals(1, testObject.production(IntStream.iterate(1, i -> -i).limit(5)));
        assertEquals(-120, testObject.production(IntStream.iterate(-1, i -> -(abs(i) + 1)).limit(5)));
        assertEquals(-128, testObject.production(IntStream.iterate(1, i -> -2).limit(8)));
    }

    @Test
    public void testOddSum() {
        assertEquals(9, testObject.oddSum(IntStream.iterate(1, i -> i + 1).limit(5)));
        assertEquals(4, testObject.oddSum(IntStream.iterate(0, i -> i + 1).limit(5)));
        assertEquals(1, testObject.oddSum(IntStream.iterate(1, i -> -i).limit(5)));
        assertEquals(-9, testObject.oddSum(IntStream.iterate(-1, i -> -(abs(i) + 1)).limit(5)));
        assertEquals(-7, testObject.oddSum(IntStream.iterate(1, i -> -(abs(i) + 1)).limit(5)));
        assertEquals(0, testObject.oddSum(IntStream.iterate(0, i -> -2).limit(8)));
    }
    ...
    public void testSumByRemainder() {
        ...
        assertEquals(
                Map.of(0, 15),
                testObject.sumByRemainder(1, IntStream.iterate(1, i -> i + 1).limit(5))
        );
        assertEquals(
                Map.of(0, 6,
                        1, 9),
                testObject.sumByRemainder(2, IntStream.iterate(1, i -> i + 1).limit(5))
        );
        assertEquals(
                Map.of(0, 3,
                        1, 5,
                        2, 7),
                testObject.sumByRemainder(3, IntStream.iterate(1, i -> i + 1).limit(5))
        );
        assertEquals(
                Map.of(0, 4,
                        1, 6,
                        2, 2,
                        3, 3),
                testObject.sumByRemainder(4, IntStream.iterate(1, i -> i + 1).limit(5))
        );
        assertEquals(
                Map.of(0, 5,
                        1, 1,
                        ...
                        3, 3,
                        4, 4),
                testObject.sumByRemainder(5, IntStream.iterate(1, i -> i + 1).limit(5))
        );
        assertEquals(
                Map.of(5, 5,
                        1, 1,
                        ...
                        3, 3,
                        4, 4),
                testObject.sumByRemainder(6, IntStream.iterate(1, i -> i + 1).limit(5))
        );
        assertEquals(
                Map.of(1, 1,
                        3, 1748,
                        ...
                        5, 157,
                        6, 48),
                testObject.sumByRemainder(7, IntStream.iterate(1, i -> i + (i + 19) / 2).limit(10))
        );
        assertEquals(
                Map.of(0, 21,
                        1, 15,
                        2, 21),
                testObject.sumByRemainder(3, IntStream.iterate(0, i -> (i + 1) % 7).limit(20))
        );
    ...
    public void testTotalScores() {
        assertEquals(
                ...
                testObject.totalScores(programmingResults(new Random(42))));
        assertEquals(
                ...
                testObject.totalScores(programmingResults(new Random(45682))));
        assertEquals(
                Map.of(new Person("Johnny", "Lodbrok", 26), 62.25,
                        new Person("Ragnar", "Silverhand", 30), 58.75,
                        new Person("Betty", "Eco", 33), 56.75),
                testObject.totalScores(historyResults(new Random(85364))));
        assertEquals(
                Map.of(new Person("Betty", "Eco", 22), 60.0,
                        new Person("Ragnar", "Silverhand", 36), 62.5,
                        new Person("Johnny", "Lodbrok", 26), 49.0),
                testObject.totalScores(historyResults(new Random(986513))));
    }

    @Test
    public void testAverageTotalScores() {
        assertEquals(80.66666666666667, testObject.averageTotalScore(programmingResults(new Random(8624))), 0.001);
        assertEquals(76.66666666666667, testObject.averageTotalScore(programmingResults(new Random(9513))), 0.001);
        assertEquals(54.666666666666664, testObject.averageTotalScore(historyResults(new Random(8456))), 0.001);
        assertEquals(52.166666666666664, testObject.averageTotalScore(historyResults(new Random(9821))), 0.001);
    }

    public void assertMapEquals(Map<String, Double> expected, Map<String, Double> actual) {
        assertEquals(expected.size(), actual.size(), "Map sizes don't match!");
        for (String key : expected.keySet()) {
            assertEquals(expected.get(key), actual.get(key), 0.0001, "problem with task " + key);
        }
    }
    ...
    public void testAverageScoresPerTask() {
        ...
        Map<String, Double> received1 = testObject.averageScoresPerTask(programmingResults(new Random(969)));
        ...
        Map<String, Double> received2 = testObject.averageScoresPerTask(programmingResults(new Random(654)));
        ...
        Map<String, Double> received3 = testObject.averageScoresPerTask(historyResults(new Random(853)));
        ...
        Map<String, Double> received4 = testObject.averageScoresPerTask(historyResults(new Random(753)));
    ...
    public void testDefineMarks() {
        assertEquals(
                ...
                testObject.defineMarks(programmingResults(new Random(4))));
        ...
        assertEquals(
                ...
                testObject.defineMarks(programmingResults(new Random(1))));
        assertEquals(
                Map.of(new Person("Johnny", "Paige", 35), "C",
                        new Person("Betty", "Lodbrok", 27), "D",
                        new Person("Ragnar", "Eco", 34), "D"
                ),
                testObject.defineMarks(programmingResults(new Random(7848))));
        assertEquals(
                Map.of(new Person("Umberto", "Eco", 20), "C",
                        new Person("Ragnar", "Lodbrok", 26), "E",
                        new Person("Betty", "Paige", 37), "E"
                ),
                testObject.defineMarks(programmingResults(new Random(52145))));
        assertEquals(
                Map.of(new Person("Johnny", "Lodbrok", 29), "F",
                        new Person("Umberto", "Paige", 24), "F",
                        new Person("Betty", "Eco", 28), "F"
                ),
                testObject.defineMarks(historyResults(new Random(56985))));
        assertEquals(
                Map.of(new Person("Ragnar", "Eco", 31), "F",
                        new Person("Johnny", "Paige", 31), "F",
                        new Person("Umberto", "Silverhand", 19), "E"
                ),
                testObject.defineMarks(historyResults(new Random(753))));
    }

    @Test
    public void testEasiestTask() {
        assertEquals("Lab 3. File Tree",
                testObject.easiestTask(programmingResults(new Random(969))));
        assertEquals("Lab 1. Figures",
                testObject.easiestTask(programmingResults(new Random(654))));
        assertEquals("Shieldwalling",
                testObject.easiestTask(historyResults(new Random(853))));
        assertEquals("Shieldwalling",
                testObject.easiestTask(historyResults(new Random(7))));
    }
    ...
    public void testToPrintableStringCollector() {
        assertEquals(
                ...
                programmingResults(new Random(3578964))
                        .collect(testObject.printableStringCollector()));
        assertEquals(
                "Student           | Lab 1. Figures | Lab 2. War and Peace | Lab 3. File Tree | Total | Mark |\n" +
